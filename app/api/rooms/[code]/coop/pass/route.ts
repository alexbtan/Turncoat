...
import { coopPass } from "@/lib/gameCoop";
import { error, json, loadRoom, readBody } from "@/lib/http";
import { store } from "@/lib/store";

export const dynamic = "force-dynamic";

// Co-op guesser toggles a vote to end the round. Body: { playerId }.
export async function POST(
  ...
  const result = coopPass(room, player);
  if (!result.ok) return error(result.error ?? "Could not end round.");

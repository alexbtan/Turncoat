import { findPlayer, guessCard, toClientRoom } from "@/lib/game";
import { error, json, loadRoom, readBody } from "@/lib/http";
import { store } from "@/lib/store";

export const dynamic = "force-dynamic";

// Operative reveals a card. Body: { playerId, index }.
export async function POST(
  ...
  const result = guessCard(room, player, Number(body.index));
  if (!result.ok) return error(result.error ?? "Invalid guess.");
